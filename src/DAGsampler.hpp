...
struct Parameters {
	...
	string info(){
		...
		infos += " -a " + TOS(a) + " -d " + TOS(d) + " -e " + TOS(e) + " -p " + TOS(p) + " -s " + TOS(s); 
...
class DAGsampler {
	public:
		...
		void	sMBR (int t, bool disjoint);				// The Markov blanket resampling potentially not requiring disjointness.
		void	sMBR_alt (int t, bool disjoint);			// The Markov blanket resampling potentially not requiring disjointness.
...
void DAGsampler::para(const string& params){
    ...
    // Use a map to map options to their corresponding handler functions.
    map< string, function< void(const string&) > > option_handlers;
    option_handlers["-a"] = [&](const string& value) { parameters.a = stoi(value); };    
    option_handlers["-e"] = [&](const string& value) { parameters.e = stof(value); };
    option_handlers["-d"] = [&](const string& value) { parameters.d = stoi(value); };    
    option_handlers["-p"] = [&](const string& value) { parameters.p = stoi(value); };    
    option_handlers["-s"] = [&](const string& value) { parameters.s = stoi(value); };        
	option_handlers["-P"] = [&](const string& value) { parameters.P = stoi(value); };   
	option_handlers["-K"] = [&](const string& value) { parameters.K = stoi(value); };   
	option_handlers["-M"] = [&](const string& value) { parameters.M = stoi(value); };   
	option_handlers["-R"] = [&](const string& value) { parameters.R = stoi(value); };  
	option_handlers["-I"] = [&](const string& value) { parameters.I = value; }; 
	option_handlers["-O"] = [&](const string& value) { parameters.O = value; };  
	 
    // Parse tokens from the stringstream.
    while (iss >> token) {
        if (option_handlers.find(token) != option_handlers.end()) {
            string value;
            if (iss >> value) {
                // Call the corresponding function with the argument.
                option_handlers[token](value);
            } else cerr << "Error: Missing value for option " << token << endl;
        } else cerr << "Error: Unknown option " << token << endl;
    }
}
...
void DAGsampler::sMBR(int t, bool disjoint){
	using namespace std;
...
void DAGsampler::sMBR_alt(int t, bool disjoint){
	using namespace std;
	...
	while (t--){
		...
		// Draw a random permutation of the children of i.
		vint rCi = dagp->C[i]; rperm(rCi);
		// Draw aalso the backward permutation.
